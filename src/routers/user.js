...
router.patch('/me', auth, async (req, res) => {
  ...
  try {
    ...
    if (err.message.includes('User validation failed')) {
      ...
      res.status(400).send({ error: 'Cannot process request' });
    }
  }
});
...
router.post('/me/avatar', auth, upload.single('avatar'), (err, req, res, next) => {
  res.status(400).send({ error: err.message });
}, async (req, res) => {
  if (!req.file) {
...
router.get('/:id/avatar.png', async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
