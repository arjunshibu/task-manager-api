...
const sendWelcome = (email, name, token) => {
  const msg = {
    ...
    html: `<b>Welcome to our App, ${name}.</b><br><br>Please verify your account by following this link<br>${process.env.URL}/users/verify/${token}<br>Link is valid only for 1 hour.`
  };
  sgMail.send(msg);
}
